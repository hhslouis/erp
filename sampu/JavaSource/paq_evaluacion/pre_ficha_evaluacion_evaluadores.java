...
if(tab_competencias.getTotalFilas()>0){
	...
	public void calcularCumplimiento(AjaxBehaviorEvent evt){
		...
		int meta=Integer.parseInt(tab_actividad_puesto.getValor("META_EVACP"));
		int cumplido=Integer.parseInt(tab_actividad_puesto.getValor("CUMPLIDO_EVACP"));
		if(cumplido<=meta){			
		...
		if (rep_reporte.getReporteSelecionado().equals("Resultado Evaluacion")){
			if (tab_evaluados.getTotalFilas()>0) {
				if (rep_reporte.isVisible()){
					...
					rep_reporte.cerrar();				
					p_parametros.put("IDE_EVDES",Integer.parseInt(tab_evaluados.getValor("IDE_EVDES")));					
