...
public class pre_anticipos extends Pantalla {
	...
	public boolean validarDatosAbonoAnticipo(){
		...
		if (Integer.parseInt(tab_anticipo_abonos.getValor("PLAZO_NRANA"))==0){
			utilitario.agregarMensajeInfo("Plazo erroneo","El plazo no puede ser cero ");
			return false;
		}		
		if (Integer.parseInt(tab_anticipo_abonos.getValor("PLAZO_NRANA"))>int_num_max_cuotas_abonos){
	...
	public void aceptarAbonos(){
		if (validarDatosAbonoAnticipo()){
			...
			if (ser_anticipo.generarAbonoAnticipo(
					...
					0, 
					Integer.parseInt(tab_anticipo_abonos.getValor("PLAZO_NRANA")), 
					30, 
					0, 
		...
		if (tab_anticipo.getValor("CALIFICADO_NRANT") ==null 
			...
			if (validarDatosAmortizacion()){
				TablaGenerica tab_amort=ser_anticipo.getTablaAmortizacion(
						...
						Integer.parseInt(tab_anticipo_interes.getValor("PLAZO_NRANI")), 
						Integer.parseInt(tab_anticipo_interes.getValor("AMORTIZACION_NRANI")), 
						Integer.parseInt(tab_anticipo_interes.getValor("MES_GRACIA_NRANI")), 
	...
	public boolean validarPlazoMaximoPagoCondicionesAnticipo(){
		...
		try {
			if (Integer.parseInt(maximo_dias_pago)<(Integer.parseInt(tab_capacidad.getValor("NRO_MES_NRCAP"))*30)){
				...
				con_datos_memo.setMessage("EL EMPLEADO NO CUMPLE CON LAS CONDICIONES DE ANTICIPOS<br>" +
						...
						"</br>PERO DE TODAS MANERAS DESEA CALIFICAR EL ANTICIPO ");
				//tab_capacidad.setValor("NRO_MES_NRCAP", (Integer.parseInt(maximo_dias_pago)/30)+"");
	...
	public void calcularValorCuota(AjaxBehaviorEvent evt){
		...
		int nro_mes_sol=0;
		try {
			nro_mes_sol=Integer.parseInt(tab_anticipo.getValor("NRO_MES_NRANT"));
		} catch (Exception e) {
			// TODO: handle exception
		}
		...
		int nro_mes_cpa=0;
		try {
			nro_mes_cpa=Integer.parseInt(tab_capacidad.getValor("NRO_MES_NRCAP"));
		} catch (Exception e) {
			// TODO: handle exception
		}
	...
	public void calcularRubrosDetallePagos(){
		...
		if (tab_rubro_detalle_pago.getValor("IDE_NRRUB")!=null && !tab_rubro_detalle_pago.getValor("IDE_NRRUB").isEmpty()){
			...
			if (tab_rubro_detalle_pago.getValor("IDE_NRRUB").equals(utilitario.getVariable("p_nrh_rubro_descuento_decimo_cuarto"))){
				...
				int num_dias=0;
				try {
					num_dias=Integer.parseInt(num_cuotas)*Integer.parseInt(amortizacion_cada);
				} catch (Exception e) {
					// TODO: handle exception
				}
	...
	public void cargarTablasPantalla(){
		...
		System.out.println("rdp "+ide_nrrdp_ant);
	...
	private boolean validarCondicionesAnticipo(){
		...
		if (minimo_dias!=null){
			try {
				if (Integer.parseInt(num_dias_trabajados)<Integer.parseInt(minimo_dias)){
			...
			if (tab_anticipo.getTotalFilas()>0) {
				if(tab_anticipo.getValor("APROBADO_NRANT").equals("true")){
					if (rep_reporte.isVisible()){
						...
						rep_reporte.cerrar();				
						//		p_parametros.put("IDE_GTEMP",Long.parseLong(tab_anticipo.getValor("IDE_GTEMP")));
						p_parametros.put("IDE_NRANT",Integer.parseInt(tab_anticipo.getValorSeleccionado()));
