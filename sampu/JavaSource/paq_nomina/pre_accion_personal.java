...
//		bar_botones.agregarBoton(bot_reactiva);
		...
		.setCombo(
				...
				"IDE_GECAF",
				"DETALLE_GECAF",
				"PRINCIPAL_SECUNDARIO_GECAF=1 AND IDE_GECAF IN (SELECT IDE_GECAF FROM GEN_GRUPO_CARGO where IDE_GEGRO=-1)");
...
public void aceptarPartidaGrupoCargo(){
	if (aut_part_gru_cargo.getValor()==null || aut_part_gru_cargo.getValor().isEmpty()){
		...
		if(tab_datos.isEmpty()==false){
			tab_empleado_departamento.getColumna("IDE_GECAF").setCombo("GEN_CARGO_FUNCIONAL", "IDE_GECAF", "DETALLE_GECAF", "PRINCIPAL_SECUNDARIO_GECAF=1 AND IDE_GECAF IN (SELECT IDE_GECAF FROM GEN_GRUPO_CARGO where IDE_GEGRO="+tab_datos.getValor("IDE_GEGRO")+")");
...
public void cambiarPartida(){
	...
	public void llenarFechaFinContrato(DateSelectEvent evt){
		...
		try {
			int_dias=Integer.parseInt(ser_gestion.getTipoContrato(tab_empleado_departamento.getValor("IDE_GTTCO")).getValor("DIA_FINC_GTTCO"));
		} catch (Exception e) {
		...
		if(tab_empleado_departamento.getValor("IDE_GTTCO")!=null){
			...
			if(!tab_tipocon.isEmpty()){
				...
				if(str_dias!=null && str_fecha_ini!=null){
					int int_dias=0;
					try {
						int_dias=Integer.parseInt(str_dias);
					} catch (Exception e) {
						// TODO: handle exception
					}
					tab_empleado_departamento.setValor("FECHA_FINCTR_GEEDP", utilitario.getFormatoFecha(utilitario.sumarDiasFecha(utilitario.getFecha(str_fecha_ini), int_dias)));
	...
	private void actualizarCombosDepartamentoEmpleado(String ide_sucu,String IDE_GEGRO,String IDE_GEARE){
		tab_empleado_departamento.getColumna("IDE_GECAF").setCombo("GEN_CARGO_FUNCIONAL", "IDE_GECAF", "DETALLE_GECAF", "PRINCIPAL_SECUNDARIO_GECAF=1 AND IDE_GECAF IN (SELECT IDE_GECAF FROM GEN_GRUPO_CARGO where IDE_GEGRO="+IDE_GEGRO+")");
	...
	private void actualizarCombosDepartamentoEmpleadoconActual(){
		tab_empleado_departamento.getColumna("IDE_GECAF").setCombo("GEN_CARGO_FUNCIONAL", "IDE_GECAF", "DETALLE_GECAF", "PRINCIPAL_SECUNDARIO_GECAF=1 AND IDE_GECAF IN (SELECT IDE_GECAF FROM GEN_GRUPO_CARGO where IDE_GEGRO="+tab_partida_vigente.getValor("IDE_GEGRO")+")");		
	...
	private void actualizarCombosDepartamentoActivoEmpleado(){
		tab_partida_vigente.getColumna("IDE_GECAF").setCombo("GEN_CARGO_FUNCIONAL", "IDE_GECAF", "DETALLE_GECAF", "PRINCIPAL_SECUNDARIO_GECAF=1 AND IDE_GECAF IN (SELECT IDE_GECAF FROM GEN_GRUPO_CARGO where IDE_GEGRO="+tab_partida_vigente.getValor("IDE_GEGRO")+")");
	...
	private String getMaximoSecuencial(int anio){
		...
		if(lis_max!=null){
			try {
				str_max= String.valueOf(Integer.parseInt(lis_max.get(0).toString())+1);
			} catch (Exception e) {
				// TODO: handle exception
				str_max="1";
			}
	...
	private void cargarTablaPartida(String IDE_GEAME,boolean boo_terminacion_subroga) {
		...
		if(IDE_GEAME!=null){
			...
			if (tab_deta_empleado_depar.isFilaInsertada() && boo_terminacion_subroga){
				tab_empleado_departamento.getColumna("IDE_GECAF").setCombo("GEN_CARGO_FUNCIONAL", "IDE_GECAF", "DETALLE_GECAF", "PRINCIPAL_SECUNDARIO_GECAF=1 AND IDE_GECAF IN (SELECT IDE_GECAF FROM GEN_GRUPO_CARGO)");	
...
//				tab_empleado_departamento.actualizarCombosFormulario();
	...
	public void dibujarContratoTerminacionSubrogacion(){
		...
		TablaGenerica tab_contrato_anterior=utilitario.consultar("SELECT * FROM GEN_EMPLEADOS_DEPARTAMENTO_PAR where IDE_GEDED in ( " +
				...
				"where IDE_GTEMP="+aut_empleado.getValor()+" and ACTIVO_GEDED=1))");
...
//			cargarTablaPartida(tab_deta_empleado_depar.getValor("IDE_GEAME"));
	...
	public void guardar() {		
		...
		if (tab_deta_empleado_depar.getTotalFilas()>1){
		String ide_geedp_activo=utilitario.consultar("select * from GEN_EMPLEADOS_DEPARTAMENTO_PAR where ide_geded in ( select ide_geded from GEN_DETALLE_EMPLEADO_DEPARTAME where IDE_GTEMP="+aut_empleado.getValor()+" and ACTIVO_GEDED=1)").getValor("ide_geedp");
