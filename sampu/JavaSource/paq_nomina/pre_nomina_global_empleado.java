...
public class pre_nomina_global_empleado extends Pantalla {
	...
	public pre_nomina_global_empleado() {        
		...
		set_det_tip_nomina.setSeleccionTabla("SELECT a.IDE_NRDTN,b.DETALLE_GTTEM,d.DETALLE_NRTIN,e.DETALLE_nrtit " +
				...
				"where a.ide_nrdtn in (select IDE_NRDTN from NRH_ROL where IDE_GEPRO=-1) ","IDE_NRDTN");
...
//					"GROUP BY a.ide_nrrub,a.detalle_nrrub ");
	...
	public void dibujarTabla(){
		...
		for(int i=0;i<(5+lis_nom_columnas.size());i++){
			Column c8=new Column();
			Etiqueta eti=new Etiqueta();

			if (i>=5){
				eti.setTitle(lis_nom_columnas.get(i-5)+"");
			}
		...
		try { 
			...
			for (int i = 0; i < lis_datos_rol.size(); i++) {
				...
				if (i==0){
					...
					// CARGO
					lab1 = new jxl.write.Label(3, 3, "CARGO", formato_celda);
					hoja_xls.addCell(lab1);
					cv=new CellView();
					cv.setAutosize(true);
					...
					// CONTRATO
					lab1 = new jxl.write.Label(4, 3, "CONTRATO", formato_celda);
					hoja_xls.addCell(lab1);
					cv=new CellView();
					cv.setAutosize(true);
					...
					for (int j = 0; j < lis_nom_columnas.size(); j++) {
						...
						jxl.write.Label lab2 = new jxl.write.Label(j+5, 3, lis_nom_columnas.get(j)+"", formato_celda);
						hoja_xls.addCell(lab2);
						cv=new CellView();
						cv.setAutosize(true);
						hoja_xls.setColumnView(j+5,cv);
					}
				}
				...
				for (int j = 0; j < lis_datos.size(); j++) {
					...
					for (int l = 0; l < lis_nom_columnas.size(); l++) {
						...
						try {
							jxl.write.Number num = new jxl.write.Number(l+5, i+int_fila+1, Double.parseDouble(String.valueOf(fila_datos[l+5])), formato_celda_valor_rubro); 
							hoja_xls.addCell(num);
						} catch (Exception e) {
							// TODO: handle exception
							jxl.write.Label lab3 = new jxl.write.Label(l+5, i+int_fila+1, fila_datos[l+5]+"", formato_celda_valor_rubro);
							hoja_xls.addCell(lab3); 
					...
					if (str_ide_sucu.equalsIgnoreCase(tab_rep_global.getValor(i, "IDE_SUCU"))){
						if (str_ide_gtemp.equalsIgnoreCase(tab_rep_global.getValor(i, "IDE_GTEMP"))){
							try {
								...
								if (indice>-1){
									obj_columnas[indice]=""+big_valor;
									totales[indice-5]=totales[indice-5]+Double.parseDouble(tab_rep_global.getValor(i, "MONTO"));
									totales_consolidado[indice-5]=totales_consolidado[indice-5]+Double.parseDouble(tab_rep_global.getValor(i, "MONTO"));
								}
							} catch (Exception e) {
							}
						}else{
