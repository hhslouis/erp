...
public class pre_flujo_efectivo extends Pantalla{
	...
	public pre_flujo_efectivo() {
		...
		tab_balance_inicial.getColumna("con_ide_flujo_efectivo").setCombo("cont_flujo_efectivo", "ide_flujo_efectivo", "des_cuenta", "");
	...
	public void generarCedula(){
		...
		if(sel_calendario.isVisible()){
			...
			String traspasa_valores="update cont_flujo_efectivo"
					+" set valor = (case when flujo =1 then debe3 when flujo = 2 then haber3 end)"
					+" from ("
					+" select ide_cocac,debe_acumulado_cobac+ debe_periodo_cobac as debe3,haber_acumulado_cobac+haber_periodo_cobac as haber3,(case when debe_saldo_cobac = 0 then haber_saldo_cobac else debe_saldo_cobac end) as nuevovalor from cont_balance_comprobacion ) a where ide_cocac=ide_cuenta";
